...
public class DConsumerKudu {
	...
	public static void main(String[] args) throws Exception {
		...
		try(KafkaConsumer<String,byte[]> consumer = new KafkaConsumer<>(properties)){            	
        	consumer.subscribe(Arrays.asList(dTopicName));//모바일로그 토픽 구독
            SpecificDatumReader<deal_log> SpecificDatumReader = new SpecificDatumReader<deal_log>(deal_log.class);
            ...
            while(true){
            	try {                
            			...
            			for(ConsumerRecord<String, byte[]> record : records) {                    	
            				...
            				deal_log read = SpecificDatumReader.read(null, binaryDecoder);                     
            				System.out.println("Message from Topic("+dTopicName+") : " + read.toString());                            					
            			}                    
